...
const ClassChild = ({ c }) => {
  ...
  return (
    <div
      ...
      className={`group
       ${
        c.type === "a" ? "a-classchild" : "classchild"
      }
                 `}
    >
      {/* Hover Gradient Overlay */}
      <div
        className="absolute inset-0 bg-gradient-to-r sm:bg-gradient-to-b from-primary/0 via-primary/5 to-primary/0 
      ...
      <div className="flex flex-col items-start sm:items-center gap-2 sm:gap-3 md:gap-4 w-full relative z-10 min-w-0">
        ...
        <div className="flex items-center sm:justify-center gap-2 md:gap-3 flex-wrap">
          {c.student_count > 0 && (
            <span
              className="flex items-center gap-1.5 px-2.5 py-1 md:px-3 md:py-1.5 rounded-full 
      ...
      <div
        className="hidden sm:block absolute -bottom-10 -right-10 w-24 h-24 md:w-32 md:h-32 lg:w-40 lg:h-40 
