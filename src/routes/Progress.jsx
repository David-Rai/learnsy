...
const LeaderboardSection = () => {
        ...
        const leadersWithStats = await Promise.all(
          boardData.map(async (leader, index) => {
            ...
            const { data: userData } = await supabase
              .from("users")
              .select("user_meta")
              .eq("id", leader.user_id) // adjust if you use auth_id instead
              .maybeSingle()
            ...
            return {
              ...
              position: index + 1,
              isCurrentUser: user?.id === leader.user_id
            }
          })
        )
  ...
  return (
    <div className="space-y-4">
      {leaders.map((leader) => (
        ...
        >
          ...
          <div className="flex items-center space-x-4">
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm ${getRankBadgeColor(
                leader.position
              )}`}
            >
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center space-x-3 text-xs text-gray-600 dark:text-gray-300">
                <span>{leader.stats.accuracy || 0}%</span>
      ...
      <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-600 flex justify-between text-sm text-gray-600 dark:text-gray-400">
        <span>Showing top {leaders.length} players</span>
        <span>Top score: {leaders[0]?.point || 0} points</span>
      </div>
    </div>
  )
}
